...
def main():
    ...
    device_auth = run.DeviceAuthentication(device_id, manufacturer)
    ...
    for ratio in negative_ratios:
        ...
        for _ in range(num_avg):
            num_samples = 100
            num_negative_samples = round(num_samples * ratio)
    ...
    fig, ax = plt.subplots()
    bar_width = 0.35
    index = np.arange(len(negative_ratios))
    ...
    ax.set_xticks(index)
    ax.set_xticklabels([f'{ratio:.1%}' for ratio in negative_ratios])
